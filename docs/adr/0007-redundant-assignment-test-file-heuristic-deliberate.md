...
- Extending this repo's own self-dogfooding config to lint `tests/` (at the time of this decision still excluded entirely via `files: ^src/`) was a distinct, separate question, not part of this decision — see `docs/adr/0009-lint-tests-directory.md`, which resolved it.
