...
def run_hook(cwd, filenames, args=None):
    """
...
def setup_test_env(tmp_path, config_content, bad_files):
...
def test_suggest_mode_http(tmp_path):
...
def test_fix_mode_http(tmp_path):
...
def test_fix_mode_file_open(tmp_path):
...
def test_suggest_mode_file_read_text(tmp_path):
...
def test_fix_mode_file_read_text(tmp_path):
...
def test_fix_mode_name_collision(tmp_path):
