...
**Reporting level:** `--forbid-vars-level={conservative,permissive}` (default `conservative`) controls whether a forbidden name with no suggested replacement gets reported at all:
...
`--fix` behaves identically at both levels: it only ever applies a high-confidence suggestion, never a weaker one and never a name with no suggestion at all.
